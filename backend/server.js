...
io.on("connection", (socket) => {
  ...
  socket.on("joinGame", ({ gameId, username }) => {
    ...
    if (users[socket.id]?.gameId) {
      ...
      games[oldGameId].players = games[oldGameId].players.filter(
      ...
      if (games[oldGameId].players.length === 0) {
        ...
        io.to(oldGameId).emit("gameJoined", {
          gameId: oldGameId,
          players: games[oldGameId].players,
        });
      }
  ...
  socket.on("solveMath", async ({ gameId, username, answer }) => {
    ...
    if (parseInt(answer) === games[gameId].mathProblem.correctAnswer) {
      ...
      console.log(
        `${username} solved the problem correctly in room: ${gameId}`
      );
      ...
      try {
        ...
        const loser = await User.findOne({
          name: games[gameId].players.find((p) => p.username !== username)
            ?.username,
        });
  ...
  socket.on("findPair", async (username) => {
    if (waitingPair.length > 0) {
      ...
      socket.emit("waitingForPair", {
        message: "Finding a pair, please wait...",
      });
    }
  });
  ...
  socket.on("submitExpression", async ({ gameId, expression, username }) => {
    ...
    console.log(
      `[SUBMIT] Received expression: ${expression} from ${username} in game: ${gameId}`
    );
    ...
    try {
      ...
      if (expressionDigits !== sequenceDigits) {
        throw new Error(
          `Digits in expression do not match the sequence "${game.sequence.sequence}"`
        );
      }
      ...
      if (result === 100) {
        ...
        try {
          ...
          const loser = otherPlayer
            ? await User.findOne({ name: otherPlayer.username })
            : null;
